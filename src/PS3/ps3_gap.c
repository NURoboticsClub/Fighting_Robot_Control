...
#include "../common/stack/gap_api.h"
#include "../common/stack/bt_types.h"
#include "../common/osi/allocator.h"
...
uint16_t ps3_gap_handle_hidc = GAP_INVALID_HANDLE;
uint16_t ps3_gap_handle_hidi = GAP_INVALID_HANDLE;
...
{
    ps3_gap_handle_hidc = ps3_gap_init_service( "PS3-HIDC", BT_PSM_HIDC, BTM_SEC_SERVICE_FIRST_EMPTY   );
    ps3_gap_handle_hidi = ps3_gap_init_service( "PS3-HIDI", BT_PSM_HIDI, BTM_SEC_SERVICE_FIRST_EMPTY+1 );
}
...
void ps3_gap_send_hid( hid_cmd_t *hid_cmd, uint8_t len )
{
    ...
    result = GAP_ConnBTWrite(ps3_gap_handle_hidc, p_buf);
...
static void ps3_gap_update_connected()
{
    is_connected = GAP_ConnGetL2CAPCid(ps3_gap_handle_hidc) != 0
                && GAP_ConnGetL2CAPCid(ps3_gap_handle_hidi) != 0;
}
